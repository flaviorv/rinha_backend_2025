...
async function main() {
  ...
  async function processPayment(processor:string, payment: Payment) {
    ...
    if (response.ok) {
      await redis.zadd(`payment:${processor}`, currentDate.getTime(), `${payment.correlationId}:${payment.amount}`)
    }
    return response
  }
  ...
  app.post('/payments', async (c) => {
    ...
    for (const processor of PROCESSORS) {
      const res = await processPayment('default', body);
      if (res.ok) {
        return res
      }
    }
  ...
  app.get('/payments-summary', async (c) => {
    ...
    const totalAmountDefault = defaultPayments.reduce((sum: Decimal, val: string) => sum.plus(Decimal(val.split(':')[1])), new Decimal(0))
    ...
    const totalAmountFallback = fallbackPayments.reduce((sum: Decimal, val: string) => sum.plus(Decimal(val.split(':')[1])), new Decimal(0))
    return c.json({
      default: {
  ...
  app.put('/admin/configurations/failure', async(c) => {
    ...
    const defaultResponse = await fetch('http://payment-processor-default:8080/admin/configurations/failure', {
      method:'PUT',
      headers: {'Content-Type': 'application/json', 'X-Rinha-Token': '123'},
      body: rawBody
    }) 
    const fallbackResponse = await fetch('http://payment-processor-fallback:8080/admin/configurations/failure', {
      method:'PUT',
      headers: {'Content-Type': 'application/json', 'X-Rinha-Token': '123'},
      body: rawBody
    }) 
    const defaultBody = await defaultResponse.json()
    const fallbackBody = await fallbackResponse.json()
    return new Response(JSON.stringify({defaultBody, fallbackBody}), {status: defaultResponse.status})
  })

  app.get('/status', async () => {
    const defaultResponse = await fetch('http://payment-processor-default:8080/payments/service-health')
    const fallbackResponse = await fetch('http://payment-processor-default:8080/payments/service-health')
    const defaultBody = await defaultResponse.json()
    const fallbackBody = await fallbackResponse.json()
    return new Response(JSON.stringify({defaultBody, fallbackBody}), {status: defaultResponse.status})
  })
